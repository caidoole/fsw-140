...
cardRouter.get('/createTable', (req, res) => {
    const sql = "CREATE TABLE cards (id INT AUTO_INCREMENT, userid VARCHAR(250), numbercollected INT, name VARCHAR(100), color VARCHAR(25), manacost VARCHAR(100), type VARCHAR(100), rarity VARCHAR(25), abilities VARCHAR(250), flavor VARCHAR(250), power INT, toughness INT, expansion VARCHAR(100), artist VARCHAR(100), collectornumber INT, artstyle VARCHAR(100), imageurl VARCHAR(250), PRIMARY KEY (id));"
    //Execute the SQL Command
...
//Execute INSERT
//Add a card with username as userid
...
//Execute a SELECT Query 
//Select all cards for a specific user
cardRouter.get('/getcards/', (req, res) => {
    const sql = `SELECT * FROM cards WHERE userid = '${req.user.username}'`;
    //Execute the SQL Command
    magicDB.query(sql, (err, result) => {
        if (err) {
            throw err;
...
//Update a card
cardRouter.put('/updatecollected/:id', (req, res) => {
    const updateCard = req.body;
    const sql = `UPDATE cards SET ? WHERE id = ${req.params.id}`;
    //Execute the SQL Command
    magicDB.query(sql, updateCard, (err, result) => {
        if (err) {
            throw err;
        }
        res.status(201).send(result);
...
//Delete a card
cardRouter.delete('/deletecard/:id', (req, res) => {
